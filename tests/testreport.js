...
(async () => {
  ...
  // ---------- ① 後から入った人が回答できるか ----------
  // 偽サーバはsessionStorageに置いてあるので、開き直しのタイミングによっては
  // まだ読めていないことがある（本物のサーバでは起きない、テスト側の都合）。
  for (let i=0;i<4;i++) {
    await p.goto(base+'?a='+i+'#g='+gid); await p.waitForTimeout(1200);
    if (await p.evaluate(()=>state.view==='group')) break;
  }
  // 自分の名前が未選択のときは入力欄が最初から出ている。出ていなければ開く
  const addBtn = p.locator('button:has-text("別の人を追加する")');
  if (await addBtn.count()) { await addBtn.first().click(); await p.waitForTimeout(200); }
